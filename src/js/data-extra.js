...
const diaryEntries = [
  { date: '2025-04-15', hrv: 45, note: 'Tuntui väsyneeltä, paljon töitä.' },
  { date: '2025-04-14', hrv: 60, note: 'Kävin lenkillä, olo oli hyvä.' },
  { date: '2025-04-13', hrv: 30, note: '' }
];

let showOnlyWithNotes = false;

function getStressLevel(hrv) {
  if (hrv == null) return '-';
  if (hrv >= 50) return 'Matala';
  if (hrv >= 40) return 'Kohtalainen';
  return 'Korkea';
}

function getRowClass(hrv) {
  if (hrv == null) return '';
  if (hrv >= 50) return 'low-stress';
  if (hrv >= 40) return 'medium-stress';
  return 'high-stress';
}

function renderDiaryTable(filtered = false) {
  ...
  diaryEntries.forEach(entry => {
    ...
    if (!filtered || hasNote) {
      ...
      const stressLevel = getStressLevel(entry.hrv);
      const rowClass = getRowClass(entry.hrv);
...
function toggleDiaryFilter() {
  showOnlyWithNotes = !showOnlyWithNotes;
  renderDiaryTable(showOnlyWithNotes);
}

document.addEventListener('DOMContentLoaded', () => renderDiaryTable());
